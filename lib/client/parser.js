...
function parse (cmd, data) {
    ...
    switch (cmd) {
        ...
        case 'auth' :
            if (!data) return error(501, 'incomplete auth command');

            var mplain = data.match(/^plain\s+(\S+)/i);
            var mlogin = data.match(/^login\s*(\S+)?/i);
            if(mplain) {
                var digest = new Buffer(mplain[1], 'base64').toString('ascii').split('\0');
                res.user = digest[1];
                res.password = digest[2];
            }
            else if(mlogin) {
                res.user = mlogin[1];
            }
            else return error(501, 'parse error in auth command');
